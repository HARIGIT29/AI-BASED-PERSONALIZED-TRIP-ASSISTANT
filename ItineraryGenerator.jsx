...
const ItineraryGenerator = () => {
  ...
  const optimizeRoutes = async (itineraryData) => {
    ...
    if (isChennaiCoords && !isChennai) {
      console.error('❌ Accommodation has Chennai default coordinates but destination is not Chennai:', tripDetails?.destination);
      console.error('📍 Accommodation coordinates:', accommodationCoords);
      console.error('📍 Accommodation data:', accommodation);
      console.warn('⚠️ Skipping route optimization due to invalid accommodation coordinates');
      return;
    }

    console.log('📍 Accommodation coordinates:', accommodationCoords);
    ...
    try {
      ...
      const routePromises = itineraryData.map(async (day, dayIndex) => {
        ...
        console.log(`🗺️ Optimizing route for day ${dayIndex + 1} with ${attractions.length} attractions`);
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.error(`❌ Route optimization failed for day ${dayIndex + 1}:`, response.status, errorText);
            return { dayIndex, route: null };
          }
        } catch (error) {
          console.error(`❌ Error optimizing route for day ${dayIndex + 1}:`, error);
          return { dayIndex, route: null };
        }
      });
      ...
      results.forEach(({ dayIndex, route }) => {
        console.log(`\n🔍 Processing route for day ${dayIndex + 1}:`, {
          hasRoute: !!route,
        ...
        if (route && route.route && Array.isArray(route.route) && route.route.length > 0) {
          ...
          if (!isAccommodationChennai || isChennai) {
            ...
            console.log(`   📍 Starting point (accommodation): [${accommodationCoords[0]}, ${accommodationCoords[1]}]`);
          } else {
              ...
              if (Array.isArray(fromCoords) && fromCoords.length === 2 && 
                ...
                if ((!isChennaiCoords || isChennai)) {
                  ...
                  if (!lastPoint || lastPoint.lat !== fromCoords[0] || lastPoint.lng !== fromCoords[1]) {
                    ...
                    console.log(`      ➕ Added from point: [${fromCoords[0]}, ${fromCoords[1]}]`);
                  }
                } else {
                  console.warn(`      ⚠️ Skipped Chennai default coordinates for 'from' point in non-Chennai destination`);
                }
              }
            }
            ...
            if (segment.to && segment.to.coordinates) {
              ...
              if (Array.isArray(toCoords) && toCoords.length === 2 && 
                ...
                if (!isChennaiCoords || isChennai) {
                  ...
                  console.log(`      ➕ Added to point: [${toCoords[0]}, ${toCoords[1]}]`);
                } else {
                  console.warn(`      ⚠️ Skipped Chennai default coordinates for non-Chennai destination`);
                }
              }
              ...
              if (Array.isArray(attrCoords) && attrCoords.length === 2 && 
                ...
                if (!isChennaiCoords || isChennai) {
                  ...
                  console.log(`      ➕ Added attraction point: [${attrCoords[0]}, ${attrCoords[1]}]`);
                } else {
                  console.warn(`      ⚠️ Skipped Chennai default coordinates for non-Chennai destination`);
                }
              }
          ...
          if (route.route.length > 0) {
            ...
            if ((!lastPoint || lastPoint.lat !== accommodationCoords[0] || lastPoint.lng !== accommodationCoords[1]) && (!isAccommodationChennai || isChennai)) {
              path.push({ lat: accommodationCoords[0], lng: accommodationCoords[1] });
              console.log(`   📍 Added return to accommodation: [${accommodationCoords[0]}, ${accommodationCoords[1]}]`);
            } else if (isAccommodationChennai && !isChennai) {
              console.warn(`   ⚠️ Skipped return to accommodation - Chennai default coordinates for non-Chennai destination`);
            }
          }
          
          if (path.length > 1) {
            polylinesMap[dayIndex] = path;
            console.log(`✅ Day ${dayIndex + 1} route path: ${path.length} points (blue line will be drawn)`);
            console.log(`   🔵 Full path:`, path);
          } else {
            console.warn(`⚠️ Day ${dayIndex + 1}: Path has only ${path.length} point(s), cannot draw line`);
          }
        } else {
          console.warn(`⚠️ Day ${dayIndex + 1}: No valid route data`);
        }
      });
      ...
      console.log('📍 Route polylines ready:', Object.keys(polylinesMap).length, 'days with routes');
      console.log('🔵 Polylines data:', polylinesMap);
    } catch (error) {
  ...
  useEffect(() => {
    console.log('🔵 Route polylines state updated:', Object.keys(routePolylines).length, 'days');
    console.log('🔵 Route polylines data:', routePolylines);
    console.log('🔵 Map loaded:', isMapLoaded);
    console.log('🔵 Itinerary length:', itinerary.length);
    console.log('🔵 Selected accommodations:', selectedAccommodations.length);
  ...
  const generateInitialItinerary = () => {
    ...
    try {
      ...
      newItinerary.forEach((day, dayIndex) => {
        if (day.activities && day.activities.length > 0) {
          ...
          if (path.length > 1) {
            ...
            console.log(`📍 Created basic route path for day ${dayIndex + 1}: ${path.length} points`);
          }
        }
      });
      ...
      if (selectedAccommodations.length > 0 && selectedAttractions.length > 0) {
        ...
        console.log('ℹ️ Route optimization skipped - no accommodations selected, using basic route');
      }
    } catch (error) {
              ...
              >
                ...
                {isMapLoaded && window.google && window.google.maps && Object.entries(routePolylines).map(([dayIndexStr, path]) => {
                  ...
                  if (path && Array.isArray(path) && path.length > 1) {
                    console.log(`🎨 Drawing optimized blue route for day ${dayIndex}:`, path.length, 'points');
